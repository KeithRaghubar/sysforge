...
package() {
    ...
    # /var/cache/sysforge holds the regenerable PGO profdata store, written
    # by the unprivileged toolchain build — 0777 so makepkg can populate it
    # without sudo (mirrors the runtime sudo-provisioning fallback).
    install -Dm644 /dev/null "$pkgdir/usr/lib/tmpfiles.d/sysforge.conf"
    {
        ...
        printf 'd /var/cache/sysforge 0777 root root -\n'
