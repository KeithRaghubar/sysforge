...
_prompt_int() {
    ...
    while true; do
        read -r -p "  $label [$default]: " value
        value="${value:-$default}"
        [[ "$value" =~ ^[1-9][0-9]*$ ]] && { echo "$value"; return; }
...
SHELL_CHOICE=$(_prompt_choice "Default shell" "bash" "bash" "zsh")
PARALLEL_DOWNLOADS=$(_prompt_int "pacman ParallelDownloads" "5")
...
_field "Default shell"      "$SHELL_CHOICE"
_field "Parallel downloads" "$PARALLEL_DOWNLOADS"
...
# Render the PKGBUILD-shipped template, substituting collected values per
# (section, key). Inheriting structure from the installed example file means
# new fields added to the template flow through without editing this script.
TEMPLATE=/usr/share/sysforge/bootstrap.toml.example
[[ -f "$TEMPLATE" ]] || _die "Template not found at $TEMPLATE — installed sysforge package may be too old (need >= v1.1.0)"

# _toml_escape produces a TOML basic string (quoted). PARALLEL_VAL stays a
# bare integer literal so the loader sees an int, not a string.
DEVICE_VAL=$(_toml_escape "$DEVICE")
ROOT_FS_VAL=$(_toml_escape "$ROOT_FS")
HOSTNAME_VAL=$(_toml_escape "$HOSTNAME")
LOCALE_VAL=$(_toml_escape "$LOCALE")
TIMEZONE_VAL=$(_toml_escape "$TIMEZONE")
KEYMAP_VAL=$(_toml_escape "$KEYMAP")
SHELL_VAL=$(_toml_escape "$SHELL_CHOICE")
USERNAME_VAL=$(_toml_escape "$USERNAME")
USER_PASSWORD_VAL=$(_toml_escape "$USER_PASSWORD")
ROOT_PASSWORD_VAL=$(_toml_escape "$ROOT_PASSWORD")
PARALLEL_VAL="$PARALLEL_DOWNLOADS"
# shellcheck disable=SC2089  # literal quotes are intentional — env var carries
# a TOML basic-string literal that Python reads verbatim from os.environ.
TARGET_VAL='"/mnt"'

if [[ -n "$COUNTRY" ]]; then
    COUNTRIES_VAL="[$(_toml_escape "$COUNTRY")]"
else
    COUNTRIES_VAL=""   # leave the template's commented placeholder alone
fi

export DEVICE_VAL ROOT_FS_VAL HOSTNAME_VAL LOCALE_VAL TIMEZONE_VAL \
    KEYMAP_VAL SHELL_VAL USERNAME_VAL USER_PASSWORD_VAL ROOT_PASSWORD_VAL \
    PARALLEL_VAL COUNTRIES_VAL TEMPLATE
# shellcheck disable=SC2090
export TARGET_VAL

python3 - > /etc/sysforge/bootstrap.toml <<'PY'
import os, re

overrides = {
    ("",          "target"):              os.environ["TARGET_VAL"],
    ("partition", "device"):              os.environ["DEVICE_VAL"],
    ("partition", "root_fs"):             os.environ["ROOT_FS_VAL"],
    ("system",    "hostname"):            os.environ["HOSTNAME_VAL"],
    ("system",    "locale"):              os.environ["LOCALE_VAL"],
    ("system",    "timezone"):            os.environ["TIMEZONE_VAL"],
    ("system",    "keymap"):              os.environ["KEYMAP_VAL"],
    ("system",    "shell"):               os.environ["SHELL_VAL"],
    ("system",    "parallel_downloads"):  os.environ["PARALLEL_VAL"],
    ("system",    "username"):            os.environ["USERNAME_VAL"],
    ("system",    "user_password"):       os.environ["USER_PASSWORD_VAL"],
    ("system",    "root_password"):       os.environ["ROOT_PASSWORD_VAL"],
}
if os.environ.get("COUNTRIES_VAL"):
    overrides[("mirror", "countries")] = os.environ["COUNTRIES_VAL"]

key_re = re.compile(r'^\s*#?\s*([a-zA-Z_][a-zA-Z_0-9]*)\s*=')
