...
Default groups (each runs in order, all surface to a single summary):
    ...
    manpage          `make man` regen output matches the committed
                     man/sysforge.1 (header date normalised). Skipped with a
                     warning if argparse-manpage is not installed.
...
def _import_build_parser(repo: Path):
    ...
    argparse-manpage and inherits the env) see the user's original setting.
    """
    saved_env = os.environ.get("SYSFORGE_CONFIG_DIR")
    saved_modules = {k: v for k, v in sys.modules.items() if k.startswith("sysforge")}
...
def check_manpage(repo: Path) -> list[Finding]:
    ...
    if not _which("argparse-manpage"):
        return [Finding("manpage", "warn", "man/sysforge.1",
                        "argparse-manpage not installed - skipping regen-diff")]

    with tempfile.NamedTemporaryFile(mode="w", suffix=".1", delete=False) as tmp:
    ...
    try:
        ...
        # uses whatever env the user has set. Overriding here would produce a
        # spurious diff against the committed man page (regenerated by
        # `make man` in the same shell).
        res = subprocess.run(
            ["argparse-manpage",
             "--module", "sysforge.cli",
        ...
        if res.returncode != 0:
            return [Finding("manpage", "error", "man/sysforge.1",
                            f"argparse-manpage failed: {res.stderr.strip()[:200]}")]
        regen = _TH_DATE_RE.sub(r'\1 "DATE"', out_path.read_text(), count=1)
