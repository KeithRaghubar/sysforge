...
**Migration.** The flat flags were removed in 3.0.0. `doctor_migration_hint(argv)` intercepts each removed flag before argparse runs and prints its replacement (exit 2) — argparse's own "unrecognized arguments" says nothing about where a flag went. `_DOCTOR_MIGRATION` holds the table; **both it and the `cli.py` hook are deleted in 3.1.0.** Two structural tests guard it: one against the hand-listed removed flags, one derived from `_SYSTEM_AXIS_ORDER` so a *new* axis added before the deletion also fails without a row.
