...
SysForge manages the profiled AUR-helper surface (install, update, and manage AUR and custom packages with system-tuned profiled builds) and a full bootstrap pipeline (stages 1–4: partition, base install, hardware detection, configure) that automates a fresh Arch install from the ISO. Current release is **<!--version-->v2.0.0<!--/version-->**; per-release changes are recorded in `docs/release-notes/`.
