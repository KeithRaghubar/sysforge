...
**Subshell toolchain env reset** (`patch_subshell_env_reset`) injects `unset CC CXX` at the top of every subshell function body (`funcname() (...)`) in `PKGBUILD.sysforge`. Subshell functions are isolated helper builds (musl bootstrap, embedded grub, dietlibc, etc.) that should use the system-default compiler, not the sysforge profile toolchain. Without this, `CC=clang` from the sysforge env leaks into sub-builds that expect gcc and produces broken toolchain wrappers (e.g. `musl-clang` instead of `musl-gcc`). Only injects when the profile sets CC/CXX to non-default values. Called from `_run_build` after PKGBUILD.sysforge is created, on all build paths (both patched and group-only).
