...
Public API: `init()`, `shutdown()`, `render(current, total, label)`, `phase(label)`, `clear()`, and a `tracker(total, prefix)` context manager that yields a `tick(label)` callable. `clear()` must be called before any `input()` prompt inside a batch loop so the prompt doesn't land inside the scroll region; the next `tick()` re-establishes the region automatically. Reservation is lazy: entering a tracker alone touches nothing — the first `tick()` call establishes the region.
