...
- sysforge install in target via `makepkg` from the source tree's PKGBUILD, run as the build user with a temporary sudoers drop-in at `/etc/sudoers.d/99-sysforge-bootstrap-build` (removed after install). The rule is scoped `NOPASSWD: /usr/bin/pacman` — the one binary `makepkg -s` needs to sync makedeps non-interactively — rather than `NOPASSWD: ALL`, because the `finally` that removes it cannot run after `SIGKILL`, an OOM kill, or power loss, and a leaked drop must not be a root-equivalent escalation primitive on a freshly installed system. Paired with an entry-time sweep of stale `99-sysforge-bootstrap-*` drops, run on the path that re-creates the file so a `--resume` after a kill cleans up its predecessor (a `pre_check` refusal would be the wrong shape: it would block the very run capable of cleaning up). The sweep is best-effort and never blocks the bootstrap. The configure stage stages the source as `sysforge-$pkgver.tar.gz` so makepkg uses the local copy instead of fetching, runs with `--skipchecksums --skipinteg` since the tarball is locally produced, and ends with sysforge owned by pacman (`pacman -Q sysforge`).
...
- **`sync_source`** is the single deliberate caller difference: `update` passes `False` (Phase 2 already synced sources through the scheduler), `build` passes `not --no-update` to keep its inline per-package source sync (which itself routes through `source_sync.get_scheduler()` inside `makepkg_wrapper.run`). `_find_existing_artifacts` and `_record_build_failure` live here too (moved from `update.py`) since both the engine and `update`'s install-only scan use them — though `_find_existing_artifacts` is now a thin delegate to `makepkg_artifacts.find_artifacts`, which `3.1.0-F9` lifted into `primitives/` so the build sandbox could share the matcher (§Primitives Layer).
...
**Phase 4.4 — Available-versions report (`--versions`).** A read-only view onto the walk that has already happened: `render_versions_report` (in `update_summary.py`, pure like `_print_result_summary`) renders a `PACKAGE / INSTALLED / AVAILABLE` table from `results` and exits before Phase 5. It shares the `--explain-drift` seam — both are list-and-exit reports, and argparse holds them in one mutually exclusive group so neither silently wins. Phase 4's own `_print_summary` is suppressed under the flag so one walk yields one report.

Only **actionable** rows are listed — `NEEDS_REBUILD`, `NEEDS_PACMAN_UPGRADE`, `DOWNGRADE`. Up-to-date packages collapse into the footer count (`190 checked · 1 available · 106 up to date`); a check *failure* (`PULL_FAILED` / `RATE_LIMITED` / `PURGE_REFUSED` / `DEVEL_EVAL_FAILED`) is not an available version and never occupies a row. Unresolved VCS packages likewise get no row: without `--devel` there is no upstream version to report, and on a devel-heavy system they outnumber real answers by roughly 80:1, recreating the noise the report exists to replace — they are surfaced as a single footer hint instead.

Stage-owned (toolchain/kernel) packages are the reason the flag exists, and they come from the **advisory** path (`_detect_stage_owned_updates`), not `--include-stage-owned`: only the advisory carries the owning stage, which is what makes such a row actionable (`[toolchain]` / `[kernel]`). The advisory is network-gated exactly as the walk is, so `--offline --versions` reports local PKGBUILD versions only and omits stage-owned rows. `--versions` is also exempt from the CLI's stale-sentinel gate (`_gate_sentinel_check`), on the same read-only grounds as `--dry-run`.
...
Flags: `--interactive`, `--packages`, `--dry-run`, `--devel`, `--offline`, `--install-only`, `--no-cleanbuild`, `--cleansrc`, `--state-dir`, `--profile-conf`, `--cache-report`, `--no-pkg-log`, `--persist-log`, `--log-dir`, `--makepkg`, `--review`, `--no-review`, `--versions`, `--explain-drift`, `--rebuild-on-toolchain-drift`, `--rebuild-on-flag-drift`, `--rebuild-on-drift`.
...
**Elapsed and ETA on the bar** (`3.2.0-F14`). The counter gives position but not pace: at default verbosity a healthy 40-second source sync and a stalled one render identically, and the user who does not run with `-v` has no other channel. While a `tracker()` is open, every painted line therefore carries a ` · <elapsed>` suffix and, once at least two items have completed, a ` · ~<eta> left` projection — `source sync · mesa · 1m04s · ~3m20s left`. The estimate is built from **measured** per-item durations in the current run, never from the configured per-operation timeouts: a timeout is a worst-case ceiling, so multiplying one by the item count advertises an hour for a run that takes forty seconds, which is both useless and a direct contradiction of the never-overstate rule `build_estimate.py` already follows. Items already finished were fetched on this machine, over this link, against this work — the best predictor available at zero cost. Three rules keep it honest: nothing is shown below a 5-second floor (short batches render byte-identically to the pre-F14 line, and the existing exact-string tracker tests are the guard); the ETA needs **two** completed items, since one sample is not a rate; and an **overrun** estimate is dropped rather than pinned at `~0s left`, because an item running past its projection is stale information, not a finished one. The suffix is composed at **paint** time, not at tick time — `render()` keeps `_last_base` suffix-free and `heartbeat()` re-appends a freshly computed suffix — so a whole package build inside a single `tick()` shows a clock that keeps moving rather than one frozen at the tick (the same freeze `3.2.0-B13` addressed for detail). `tracker()` saves and restores the previous suffix provider on entry/exit, so a nested tracker exiting does not strip an enclosing one's clock. Formatting (`42s` / `7m03s` / `2h05m`) is finer-grained than `build_estimate._fmt_hms`'s minute resolution on purpose: that one summarises a batch before it starts, this one is watched while it moves, and a clock that changes once a minute reads as stuck.
...
**How primitives reach it.** They don't import it. `ui/progress.py` registers itself with `primitives/progress_hooks.py` at import, and the leaf layer calls `progress_hooks.hooks().<name>()` — see that module below.
...
The **inverted** progress dependency (`3.2.0-F1`). Seven primitives — `prompt`, `pager`, `editor`, `makepkg_invoke`, `aur_resolve` and friends — needed exactly five names out of `ui/progress.py`: `suspend_for_prompt`, `suspended`, `reserved_rows`, `heartbeat`, `tracker`. None of them wants *the bar*; they want "tell the display I am about to take the terminal" and "count these if anyone is counting". That is a protocol, not a dependency, so this module declares it (`ProgressHooks`, `runtime_checkable`) together with a **complete no-op default** — `suspended()`/`tracker()` yield, `reserved_rows()` returns 0, the tick object carries the same `note`/`resume` shape — and `ui/progress.py` calls `progress_hooks.register(sys.modules[__name__])` at the bottom of its own module. The module object satisfies the protocol structurally, so there is no adapter to keep in sync and every lookup still resolves through `ui.progress`'s globals (patching a name there in a test behaves exactly as before). Call sites must go through `hooks()` **per use** and never bind the object at import, so a later `register` is seen; `reset()` restores the default for test teardown. The payoff is that a primitive works with no UI layer present at all, instead of transitively importing the renderer, its `SIGWINCH` handler and its `atexit` hook to ask how many rows are reserved.

### `hardware_probe.py`

Host detection and the derivations built on it: `detect_host_arch()` (uname), `parse_gpu_vendors(lspci_text)`, `derive_llvm_targets(host_arch, gpu_vendors)`, `derive_mesa_drivers(gpu_vendors)`. Split out of `pipeline/stages/hardware.py` by `3.2.0-F1` — `llvm_targets` and `mesa_drivers` need *live* detection whenever no `hardware_profile.toml` exists at the current state dir, and reaching up for it made a driver-name lookup pull in every eagerly-instantiated stage. Three-way split of responsibility: `hardware_tables.py` holds the mandatory baselines that survive any reduction, this module holds detection plus the vendor/arch derivations that consume them, and `stages/hardware.py` remains the stage (kconfig emission, profile writing, drift reporting) and re-exports these four names so `stages.hardware.derive_llvm_targets` stays a valid import path. Parsing is pure — the parsers take text and only `detect_host_arch` touches the system; running `lspci` is the caller's job.

### `paths.py`

Pure constants module — the canonical directory of every config file sysforge reads. `CONFIG_BASE` is derived from `$SYSFORGE_CONFIG_DIR` (default `/`, so system config resolves under `<base>/etc/sysforge/`, including `BOOTSTRAP_PATH`). User-side roots follow the XDG Base Directory Specification via the `_xdg_base(env, default)` helper: `USER_CONFIG_DIR` (`$XDG_CONFIG_HOME/sysforge`), `USER_CACHE_DIR` (`$XDG_CACHE_HOME/sysforge`), `USER_STATE_DIR` (`$XDG_STATE_HOME/sysforge`). The resolved path lists (`CONFIG_PATHS`, `CONFLICT_GROUP_PATHS`, `CONSUMES_INFERENCE_PATHS`) layer the user file (`$XDG_CONFIG_HOME/sysforge/…`) over the system file in `extends_system` order. Helpers: `resolve_packages_path(config)` returns the `packages.toml` path the rest of the codebase should use (honouring `--packages` overrides), and **`resolve_state_dir(cli_override)`** is the single home for state-dir resolution — `--state-dir` > `SYSFORGE_STATE_DIR` > `/var/lib/sysforge` > the XDG state dir when the default is neither writable nor provisionable — returning `(Path, source)` where `source` is one of `--state-dir`, `SYSFORGE_STATE_DIR`, `default`, `xdg-fallback`. It lived in `pipeline/state.py` until `3.2.0-F1`, which meant five primitives (`makepkg_wrapper`, `state_probe`, `source_sync`, `llvm_state`, `init_notice`) reached *upward* into the pipeline layer at function level for a path computation; `pipeline.state` re-exports the name so existing imports and test patch points are unaffected. Apart from that resolver's writability probe and its `fs_provision` fallback, the module does no I/O — just path strings.
...
- `get_repo_candidate_version(name)` — the newest repo version of `name`, tolerating a stale sync DB. `get_pacman_sync_version` reads `/var/lib/pacman/sync/*.db` and never touches the network, so on a rolling repo it reports whatever the last `pacman -Sy` left on disk — a day-old DB pins a day-old release (3.1.0-B3). This cross-checks it against `checkupdates_map()`, which refreshes a *side copy* of the sync DBs (no sudo, no partial-upgrade risk), and returns whichever is newer by `version.vercmp`. Strict enrichment: `checkupdates` only lists *installed* packages with a pending upgrade, so an absent entry, an unavailable tool, or an older value all fall back to the sync DB; `None` is returned only when the sync DB has no candidate at all. The `checkupdates` probe is memoized for the process lifetime (`reset_repo_candidate_cache()` clears it) — one probe however many packages are pinned. Sole consumer: `source_sync._pin_repo_checkout`; the diagnostic `build_diag` pkgconfig-version-gate probe deliberately keeps the raw sync-DB read, since doctor-class axes stay cheap and read-only
...
- `check_package_abi(pkg_path: Path) -> list[str]` — archive wrapper. Lists shared-object members with `bsdtar -tv`: regular files named `*.so.*` **or** unversioned `*.so` (mesa's `libgallium-<ver>.so` and drivers carry no soname suffix, and a `.so.`-only filter missed exactly the library that broke, `3.2.0-B20`), skipping symlinks and hardlinks. It extracts them with `bsdtar -x` to a temp dir, calls `check_so_files`. Invoked from `makepkg_wrapper.run()` when `--abi-check` is passed to `sysforge build`.

- `find_abi_issues(so_paths, *, benign_sink=None)` / `package_abi_findings(pkg_path)` -> `list[AbiFinding]` — the structured forms both string APIs are views of. `AbiFinding(so, kind, message, libs)` has `kind` either `"symbol"` or `"missing_needed"`. `libs` holds the resolved paths of the NEEDED libraries the requirement binds to: the ones defining its version node (the symbol vanished *within* it), else the Verneed-bound ones.
- `source_built_skew_findings(pkg_paths, source_built)` — the **blocking pre-install skew gate** (`3.2.0-B20`), called by `build_core.build_and_install` on every fresh or reused artifact before it can install or be re-injected, whatever `--abi-check` is set to. It keeps only `"symbol"` findings whose attributed lib is owned (one batched `pacman.owners_of`) by a package `build_sandbox.source_built_packages` reports. A pkgver does not describe exports: a PGO libLLVM inlines away weak `std::` copies the repo build exports under `LLVM_<ver>`, so a consumer linked against the repo build fails to load against the host's build of the same version. Scoping to source-built owners keeps repo-vs-repo findings, which pacman's dependency versioning already governs, from failing builds. Undetermined ownership does not block, the gate is inert when nothing is source-built, and a checker that cannot run warns and admits the install.
...
**Subprocess stdio:** the non-interactive branch routes makepkg's stdout+stderr through `pty_runner.run_with_pty` so child tools that gate live UI on `isatty()` (cargo's "Building [n/m]" bar, configure-script spinners) still emit their progress animation. Bytes are forwarded verbatim to `sys.stdout.buffer` when sysforge itself is on a tty so the user sees the animation alongside the bottom-anchored `[SYSFORGE][PROGRESS]` indicator. The same byte stream is decoded and split on `\n` into lines for failure classification (`prepare`/`build`/`package`), missing-dep collection (`target not found:`), already-built detection, and clang→GCC toolchain-mismatch pattern matching. Every classification (and the captured lines handed to `build_diag`) runs on `pty_runner.strip_ansi`-cleaned text: because the child sees a tty, compilers embed SGR/erase/OSC-8-hyperlink escapes *inside* diagnostic tokens — GCC ≥ 16 hyperlinks the quoted option name, which split `'-flto='` mid-token and silently defeated the mismatch patterns (gpu-burn regression, 2026-06-12) — and the matching is additionally curly-quote tolerant for localized GCC output. In verbose mode (`-vvv`) or when sysforge stdout is piped (`sysforge update | tee log.txt`), byte forwarding is suppressed; only the decoded lines reach the user, keeping captured logs free of `\r`/ANSI noise. A `MAKEPKG_HEARTBEAT_S`-cadence (default 30 s) idle callback writes `[heartbeat] <latest>` entries to the per-package log when no `\n` has crossed the boundary in that window — surfacing ninja's `\r`-redrawn `[X/Y] Building ...` status so a long compile phase doesn't look hung under `-vvv` / `sysforge log <pkg>`. **On a build failure at normal verbosity**, where the live stream only reached the terminal and the log holds only heartbeats, `invoke_makepkg` persists the last `MAKEPKG_FAILURE_TAIL_LINES` (80) of captured output to the per-package log at DEBUG (file, not console) — so the real failing block (e.g. ninja's `FAILED:` lines) is diagnosable after the fact without a `-vvv` re-run. **The window is anchored on the failure, not on the end of the stream** (`select_failure_tail`): ninja keeps building its other targets after one fails and a PGO build emits a `-Wbackend-plugin` skew warning per function, so a blind `lines[-80:]` captured 61 warnings and zero error lines on a real mesa failure, with ninja's `FAILED:` block and rustc's `error[E0605]` pushed out of it entirely (`3.2.0-B14`). The anchor is the last `FAILED:` / `error:` / `error[` / `fatal error:` line plus a few lines of lead-in; makepkg's own `==> ERROR:` epilogue is excluded from anchoring, since it matches the vocabulary and is last on every failure, but stays a fallback when nothing else was captured. An anchor already inside the last-80 window yields the old contiguous slice unchanged; otherwise the closing lines are appended too (makepkg's verdict names the failing phase) and the two segments are joined by an explicit `... N line(s) omitted ...` marker. Skipped at `-vvv` (every line is already logged) and for exit 13 (already-built, not a failure). The interactive branch still uses `subprocess.Popen` with inherited stdio so unbuffered prompts (sudo, gpg signing keys, pacman conflict resolution) reach the terminal immediately.

### `pty_runner.py`

Standalone helper: spawns a subprocess attached to a pty so child tools observe a tty on stdout+stderr. Reads raw bytes from the master fd, optionally forwards them to `sys.stdout.buffer` (preserving `\r`-based progress redraws) — verbatim except for the terminal-query filter below — and delivers decoded lines to a callback for parent-side pattern matching. Splits lines on `\n` only — `\r` is left in place mid-line so cargo's redraws aren't shredded into spurious "lines". An optional `idle_callback` fires every `idle_timeout_s` seconds when no full line has been delivered; it receives `buf.split("\r")[-1]` (the latest in-place redraw segment) or `None` if the child is silent, without consuming the buffer — subsequent `\n` still delivers the original inter-newline content unchanged. The read loop wakes via `select(master_fd, …, idle_timeout_s)`, so the heartbeat is idle-driven (no spin). Sizes the child's pty winsize to `terminal_height - reserve_bottom_rows` (caller passes `progress.reserved_rows()`): when the progress bar holds the bottom row, the child believes the screen is one row shorter and keeps its full-screen redraws/scrolling above the bar, so the bar stays visible throughout the build instead of the child scrolling over it. Handles SIGWINCH (re-applies the shrunk winsize, then chains to the previously installed handler so `ui/progress._on_sigwinch` continues to fire), EIO on child exit, and UTF-8 codepoints split across read boundaries (incremental decoder with `errors="replace"`). **When rows are reserved the child's stdin is that same pty, not the inherited terminal.** A winsize belongs to the tty *device*, not the fd, so sizing only stdout/stderr leaves stdin reporting the terminal's full height — invisible to a child that merely writes, but decisive for one that allocates a **nested** pty and seeds it from stdin. That is exactly the build sandbox (`makechrootpkg` → `sudo` → `arch-nspawn` → `systemd-nspawn --console=autopipe`, which picks its interactive console *because* stdin is a tty): the container inherited the real terminal's height, scrolled over the reserved row and wiped the bar for the whole build. Sharing the pty makes both descriptors agree so the reservation survives the extra layer; the non-interactive path runs with `--noconfirm` and is not expected to read stdin, and sharing also stops a child from stealing the user's keystrokes. With no rows reserved stdin is inherited from the parent as before, so TTY-only prompts (sudo) keep working. **That same reservation is why reply-soliciting escapes must not be forwarded** (`_TerminalQueryFilter`, `3.2.0-B17`). `systemd-nspawn` opens every container by asking the terminal for its background colour (`ESC ] 11 ; ? ST`) and pushing a window title. Forwarded verbatim, the *real* terminal answers on sysforge's `fd 0` — which nobody reads, because the asker was handed the pty slave instead — so the reply sits in the tty input queue until the line discipline echoes it back under `ECHOCTL`, printing a literal `^[]11;rgb:1b1b/1b1b/1b1b^[\` into the middle of the build output. The fix is to drop the question rather than chase the answer: OSC colour/palette queries, OSC 0/1/2 window and icon titles (the same packet renames the user's terminal tab to the container), and the CSI window-manipulation/DSR/DA finals `t`/`n`/`c`. The filter is **gated on `reserve_bottom_rows`**, not applied unconditionally: with no reservation the child owns the real stdin and can complete the exchange itself, so filtering there would break a conversation that actually works. It is display-only — `line_callback` still receives every byte, so classification and the forwarded stream cannot drift into two different notions of what the child said. Because an `os.read` boundary can fall mid-sequence, every branch requires its terminator and an incomplete trailing sequence is held until the next chunk completes it (released verbatim past `_MAX_PENDING`, so a child that opens an OSC and never closes it cannot stall the live animation), and `flush()` releases whatever is still held so the filter never loses bytes. Used by `makepkg_wrapper.py`'s non-interactive build path; reusable for any subprocess where preserving child-side ANSI animation matters. Setting **`SYSFORGE_PTY_RAW_DUMP=<path>`** appends the child's byte stream to that file *before* any filtering, led by the winsize actually applied (`rows`/`cols`/`reserve`) — a diagnostic for terminal-state bugs, which the normal log cannot serve because it is written post-`strip_ansi`, so a DECSTBM reset, `RIS` or alternate-screen switch is erased by the very filter one would search. Off unless set, and best-effort throughout (an unwritable path, a failed write or a failed close is swallowed) so a diagnostic never stops a build. Also exports `strip_ansi(text)` — removes OSC (BEL/ST-terminated, tolerating an unterminated sequence at a chunk boundary), CSI, nF charset-designation escapes (`ESC` + intermediates `0x20-0x2F` + a final byte, e.g. the `ESC ( B` that makepkg's message helpers append to every line), and two-byte C1 escapes. Any parent-side substring matching on pty-captured output must run on `strip_ansi`-cleaned text, since escape bytes can sit inside the very token being matched (GCC ≥ 16 wraps quoted option names in OSC-8 hyperlinks).
...
The module is also the one home for the **first-run stage-config advisory**: `maybe_emit_stage_config_notice(stage_name, state) -> "advised"|None`. `run toolchain` and `run kernel` are the two stages whose behaviour is decided almost entirely by their TOML rather than by the flags the user typed — `toolchain.toml` picks the compiler and the PGO passes, `kernel.toml` picks the kconfig fragments and initramfs handling — so a user who ran `setup` and then types `sysforge run kernel` would otherwise get a kernel built from defaults they have never opened. The F1 notice above cannot cover this: it keys on the post-install marker and retires itself the moment `reconfigure` and `hardware` are both `done`, i.e. it clears *before* anyone reaches toolchain or kernel. The advisory names the config file (`paths.CONFIG_DIR / <file>`) and the two or three settings most likely to surprise, driven by the `_STAGE_CONFIG_ADVICE` table; any stage absent from that table is a no-op, since a notice on every stage is a notice nobody reads. "First run" means never *completed*, not never *attempted* — a failed or interrupted first attempt re-advises, because the config is exactly as unread as it was before and a failed stage is the one most likely to be retried blind. It needs no new state: `PipelineState.stage_status` already returns `"pending"` for a stage it has never seen. Called from both pipeline entry points (`run_stage_standalone` and `run_pipeline`'s stage loop), each of which holds the state object before the stage executes. Emitted at `ui()` rather than `warn()`: it is a pre-flight block before a stage that changes the system, the same class as the plan tables `render_preflight` emits, and `warn()` is gated behind `-v` — a fresh install would never see it. Strictly advisory, matching this module's contract: it prints, never prompts, never blocks, and never raises. A prompt here would inherit the unattended-consent problems that belong to the default-on plan gate; if that gate lands, this is superseded, because a rendered plan *is* the config made visible.
...
**The profile's toolchain is installed into the chroot, not stripped.** Same category as the accelerators above — an env value naming a binary the `base-devel` chroot does not carry — and the opposite remedy, because the compiler *is* the profile's intent. A profile resolving to LLVM exports `CC=clang`/`CXX=clang++`/`LD=ld.lld` into a container holding only gcc and binutils, and every C build died in configure with `Could not find the compiler specified in the environment variable CC` — a message naming neither the sandbox nor the profile, reached only after a full source checkout, once per package (`3.2.0-B4`). `missing_toolchain` probes `_TOOLCHAIN_ENV_KEYS` against the chroot *root* copy and `provision_toolchain` installs what `_TOOLCHAIN_PACKAGE` maps them to via `arch-nspawn <root> pacman -S`, before `makechrootpkg` is spawned. Three details carry the weight: the probe resolves an **absolute** `CC` inside the chroot rather than against the host (the host has a binary at that path, which is exactly why this shipped unnoticed); the install targets the root copy, which persists, so it is one cost per chroot and not per build (`-c` resets only the working copy); and a binary with no known package is a `SandboxUnavailable` naming the `arch-nspawn` command that fixes it, since guessing a package name installs the wrong thing and continuing merely buys back the cryptic failure. Falling back to the chroot's gcc was rejected: it produces a gcc-built package under a clang profile, which is the silent downgrade the sandbox's own preflight rule exists to prevent. **The probe reads the conf, not only the exports.** Flag variables never travel in `extra_env` — that carries `CC`/`CXX` and the profile's env-type keys — they reach the container inside the conf body `chroot_conf_text` copies wholesale, so a profile selecting its linker with `LDFLAGS=-fuse-ld=lld` put it nowhere the export dict could see, the probe reported nothing missing, and every build died on `invalid linker name` with no `[SANDBOX]` line in the log to show `pacman` had never run (`3.2.0-B7`). `missing_toolchain` and `provision_toolchain` therefore take the emitted `conf_path` and read both channels; conf values are unquoted first, since `parse_system_makepkg_conf` returns them verbatim and a naive capture over `LDFLAGS="... -fuse-ld=lld"` yields `lld"`. `-fuse-ld` is the one exception to "a flag value names no binary" and the only token extracted — scanning flag strings generally would turn every `-march=native` into a package lookup.

**Profile data is mirrored into the chroot at its host path.** The same category one step further out: not a binary the container lacks but an input *file*. The PGO stages bake an absolute host path into the flags (`-fprofile-use=<store>/mesa.profdata`), and clang treats an unreadable profile as a hard error rather than a warning — which meson reports as `Compiler clang cannot compile programs`, textually identical to the `3.2.0-B4` no-compiler failure and naming neither the flag nor the file (`3.2.0-B10`). `host_profile_data` collects the input-file flags (`-fprofile-use`, `-fprofile-instr-use`, `-fprofile-sample-use`) from conf and exports alike, deduplicated because `makepkg_conf` injects one token into all three flag variables, and `provision_profile_data` copies each into the chroot **at the same absolute path** — so the already-baked token resolves unchanged in the container and no flag rewriting is needed on the way in. The `-fprofile-*generate` family is not carried (it names an output directory, so there is nothing to copy in; a container that writes profiles and is then discarded is a separate problem), a path absent on the host is skipped (nothing to copy, and the host build would fail identically), and the copy is unconditional rather than skipped when a mirror exists, since every `--pgo=use` merge rewrites the store and a stale mirror would silently pin the container to an old profile. A failed copy is a `SandboxUnavailable`: continuing hands clang a flag naming a file that is still missing, and dropping the flag would build a non-PGO package under a PGO profile — the same silent downgrade the toolchain rule above refuses.

**The progress bar's row reservation has to cross the container's own pty.** `pty_runner.run_with_pty` shrinks the child's pty by `progress.reserved_rows()` so the bar's bottom row survives a build (§Primitives Layer), but a winsize belongs to the tty *device*, not the fd — and stdin used to stay the inherited real terminal. `makechrootpkg` → `sudo` → `arch-nspawn` → `systemd-nspawn --console=autopipe` allocates a **nested** pty and sizes it from stdin, so the container ran at the terminal's full height, scrolled over the reserved row, and the indicator disappeared for the entire sandboxed build (`3.2.0-B3`). Under a reservation the child's stdin is now the same pty as its stdout, so both descriptors report the reserved height and the nesting inherits it. That rerouting has a second consequence: the container's own startup queries — `systemd-nspawn` asks the terminal for its background colour and pushes a container title — are now addressed to an fd whose answer nobody can read, and the unread reply is echoed back into the build output by the tty line discipline (`3.2.0-B17`). `run_with_pty` therefore drops those queries and title-sets from the forwarded byte stream whenever it holds the child's stdin (§Primitives Layer). `--console=autopipe` picks its interactive console *because* stdin is a tty, so the inherited terminal both triggered the extra pty layer and mis-sized it. Two opt-in diagnostics exist for the case where the bar still goes missing, because the normal logs can answer neither question: `SYSFORGE_PTY_RAW_DUMP` captures the child's stream *before* `strip_ansi` (which otherwise erases the very `ESC[r`/`RIS`/alt-screen sequences one would search for) along with the winsize actually applied, and `SYSFORGE_PROGRESS_TRACE` records every establish/release/paint/resize on sysforge's own side — separating "the region was released" from "the bar was never repainted", which look identical on screen. Both are off unless set and swallow every failure.

**Dependency scope — the standing limitation.** `arch-nspawn` gives the container its *own* `/etc/pacman.conf`, so `--syncdeps` resolves from the stock repos and never from the host's installed set. Two consequences, one closed and one still accepted:

- A locally-built dependency is invisible unless it is injected — **now seeded from two sources** (`3.1.0-F9`). The session registry covers packages built *in the same run*; `resolve_dep_artifacts` covers the rest by reading `build_state.toml`, which is keyed by **pkgname** rather than pkgbase, so split packages arrive pre-expanded in exactly the shape `-I` wants (it takes package *files*, one per pkgname). Scope is the target's dependency closure ∩ source-built, walked over the host's local DB in one pass via `pacman.get_all_package_depends` — `build_state` records versions but no dependency edges, and injecting *every* source-built package instead would copy ~150 files into the working copy per build on a stack machine. Version selection is `find_artifacts(..., exact_ver=)` against `pacman.get_installed_version`, **never** the newest artifact in `PKGDEST`: that directory is a long-lived archive of every historical build (`3.1.0-B1`), so the newest file there is routinely a version the host does not run, which would recreate the skew the injection exists to remove. A source-built dep whose artifact was pruned from the archive warns (naming the package and version) and continues — the container falls back to the repo version, a build-fidelity mismatch rather than a breach of the isolation boundary, and unlike preflight's refusal there is no matching risk to justify a hard stop. `install_args` unions the two, the session registry winning: this run's own builds are newer than anything the store points at. Two ordering and failure rules keep injection from quietly coming back empty. It is resolved **immediately after preflight**, before the canonical-name swap renames the patched sidecar to `PKGBUILD` and before the scratch conf exists. Resolving after the swap read a path that was gone and injected nothing (`3.2.0-B18`), and resolving first means a refusal leaves nothing to undo. Losing the *whole closure* **refuses** (`3.2.0-B19`): when anything is source-built, a missing PKGBUILD, a `collect_builddeps(strict=True)` parse failure or an unreadable local DB raises `SandboxUnavailable` instead of returning `[]`, which would read as "no source-built deps". One pruned artifact still warns and continues. Behind both sits the build loop's pre-install ABI skew gate (`3.2.0-B20`, §Primitives Layer → `abi_check.py`). It catches a package linked against a different build of a source-built library however that happened, because a same-pkgver skew (repo vs. PGO `llvm-libs 22.1.8-2`) is invisible to version comparison.
...
What is *not* lost is the optimization of the host's own packages: a dependent build consumes a dependency's headers and link stubs, while the optimization lives in that dependency's installed binary, which the built package still runs against on the host. With `3.1.0-F9` the sandbox reaches a host's own source-built stack rather than only AUR leaf packages whose dependencies come from repos; what remains scoped is *version agreement* against everything outside the injection set, which is `3.1.0-F10`'s local repo. It stays default-off.
