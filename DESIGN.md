...
**Mesa instrumentation PGO (`build --pgo=record|use`).** mesa is profile-guided the way a *runtime-exercised library* must be: an instrumented mesa only emits profile data when desktop/GPU apps later call into it, so the store path is baked into the build rather than discovered at runtime. The two-step flow reuses the exact flag-injection seam the toolchain PGO uses — `compiler_flags_extra`, which `emit_makepkg_conf` appends to CFLAGS/CXXFLAGS/LDFLAGS, and which mesa's `arch-meson` inherits — so there is no second injector and no meson `-Db_pgo` surgery:

- **`--pgo=record`** injects `-fprofile-generate=<store>` (store from `mesa_pgo.resolve_store` → `makepkg_pgo.resolve_method_store(method="pgo-mesa")`, provisioned `root:sysforge` setgid via `fs_provision.ensure_writable_dir`). The instrumented mesa installs over stock mesa; *any* GPU app that loads it appends `.profraw` to the store with no per-session env setup. The instrumented build is not optimized, so it keeps its stock package name (`build_mode = source_built`).
- **`--pgo=use`** merges the collected `.profraw` into one `mesa.profdata` (`mesa_pgo.merge_profraw` → `llvm-profdata`; a clean `MesaPgoError` abort if nothing was collected or the tool is missing), injects `-fprofile-use=<profdata>` (plus `-Wno-profile-instr-out-of-date`/`-unprofiled` so a `-Werror` mesa tolerates expected profile skew), and earns the `-sysforge` rename (`build_mode = pgo_mesa`). The rename is applied in `makepkg_wrapper._run_build` gated on `profile.is_optimized_build_mode` (the one home for "does this build earn the suffix?") and is `conflict` mode: `patch_package_suffix` rewrites every split member's pkgname **and its `package_<name>()` function** (or makepkg aborts at packaging time) and injects `provides`/`conflicts`/`replaces` covering the stock names, so `mesa-sysforge` cleanly swaps the stock mesa family out. `build_state` records the renamed names with `origin_pkgbase = mesa` so `update`'s source-sync still correlates back to the upstream tree.

**Profile reuse is durable across rebuilds.** mesa is source-tracked, so every `update` (and any plain `sysforge build mesa`) rebuilds it — and without re-applying the profile the user collected, that rebuild would silently regress to a stock, unprofiled mesa, contradicting the one-shot `--pgo=use`. So when a mesa rebuild runs with **no** explicit `--pgo` mode, `makepkg_wrapper._run_build` calls `mesa_pgo.reuse_profdata()`: if a merged `mesa.profdata` already exists in the store (the durable signal that this host opted into mesa PGO), it re-injects `use_flags` through the same `compiler_flags_extra` seam and re-stamps `build_mode = pgo_mesa`, so the `-sysforge` rename persists too. No re-merge happens — once `use` swaps the instrumented mesa for the optimized one, no new `.profraw` accrues, so the existing profile is current; and `use_flags` already demotes the skew warnings, so a slightly-stale profile never `-Werror`-fails the rebuild. Bare `.profraw` with no merged profdata (record-only, never `use`d) is *not* reused — there is nothing consumable yet, so the build falls back to normal. Because this path bypasses `BuildVerb.pre_check`'s LLVM gate, the reuse is itself guarded on `profile.is_llvm_toolchain` (resolved compiler: explicit override > resolved profile > env `CC`) so a clang `.profdata` is never fed to a gcc build. To opt back out, remove the store's `mesa.profdata` (or `sysforge state forget mesa`).

The whole feature is LLVM-only — it instruments with clang and merges with `llvm-profdata` — and `BuildVerb.pre_check` blocks cleanly under `toolchain = gcc` via `profile.is_llvm_toolchain` + `LLVM_REQUIRED_HINT` before any build work. `--pgo` is a logged no-op for non-mesa targets (gated on `is_mesa_pkgbase`), and lib32 mesa is excluded (the lib32 PGO flag-scrub at conf emit strips `-fprofile-*`). See *Flag/Profile System → Flag guards* and §`primitives-layer` (`mesa_pgo.py`).
