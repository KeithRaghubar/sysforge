...
|----|------|----------|--------|
| `2.6.1-F5` | Compat-alias sweep: delete the four remaining back-compat read paths | high | medium |
| `2.6.1-F6` | Normalize the parent-verb subcommand default across all namespaces | med | small |
| `2.6.1-STD2` | Deprecation policy: a declared removal version and an enforcement gate | med | medium |
...
- **`2.6.1-F5` — Compat-alias sweep: delete the four remaining back-compat read paths.** Four
  ...
  (1) `[git] pull_timeout` as a fallback for `fetch_timeout` (`update_sync.py`, undocumented and
  absent from the shipped defaults); (2) the legacy per-package key in `packages.toml`, normalized
  ...
  (3) the `build_mode = "profiled"` token normalized to `"source_built"` at the `build_state.toml`
  load point (`primitives/build_state.py`) and again in the profile layer (`primitives/profile.py`,
  whose vocabulary comment block exists solely to explain the two tokens); (4)
  `migrate_legacy_user_dirs()` (`primitives/paths.py`), invoked from `cli.py` on **every** CLI
  ...
  `state repair` migration path and an explicit release-note instruction; the other three are
  ...
  medium* — the aliases are individually trivial but each is a second live path through a
  load-bearing seam (config resolution, state parsing) that every future change must keep correct.
  **Standards home on adoption:** none new (see `2.6.1-STD2`, which supplies the policy this sweep
...
- **`2.6.1-F6` — Normalize the parent-verb subcommand default across all namespaces.** Six verb
  ...
  **breaking** and belongs in the same major that created the split. Completions
  ...
  Effort: small* — small mechanical change, but only landable in a major, and deferring it means
...
- **`2.6.1-STD2` — Deprecation policy: a declared removal version and an enforcement gate.**
  Standards row 3 covers SemVer, but only *format and cross-file version parity*
  (`tools/check_shipped.py`, `versions` group). Nothing covers **deprecation**: there is no way to
  mark a config key, state token, CLI flag or path as on its way out, nothing warns a user that the
  key they are using is scheduled for removal, and nothing tells a future session which compat paths
  are load-bearing versus vestigial. The four aliases in `2.6.1-F5` accumulated invisibly for exactly
  this reason — they were each documented correctly in a code comment, but comments scattered across
  nine files cannot be audited before a release the way a standards row wired into
  `tools/check_standards.py` can. Add a row to `docs/design/21-standards.md` defining: a single
  registry of deprecated surfaces, each carrying the version it was deprecated in and the version it
  is removed in; a warn-on-use requirement (routed through the existing `log` seam, so a user learns
  before the removal lands, not after); and a check that fails when a release ships at or past a
  declared removal version with the surface still present. `2.6.1-F5` is the first application and
  should land after it, so the registry starts populated rather than empty. *Priority: med · Effort:
  medium* — process debt, not correctness; the value is that the *next* alias is visible by
  construction instead of discoverable only by grepping for the word "legacy". **Standards home on
  adoption:** a new row in `docs/design/21-standards.md`, enforced by `tools/check_standards.py` +
  `tests/test_standards_compliance.py`.
