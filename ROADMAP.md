...
|----|------|----------|--------|------|
| `2.6.1-STD3` | register profiles.toml's patched_pkgbuild build_mode alias (standards row 24) | med | small | patch |
| `2.5.1-F1` | Kernel kconfig patcher composition: replace sentinel-tag coordination with an ordered pipeline | med | large | patch |
| `2.6.1-B3` | resolve_repo_mode's removed-token ValueError reaches the user as a traceback | low | small | patch |
...
- **`2.6.1-STD3` — register `profiles.toml`'s `patched_pkgbuild` build_mode alias (standards row
  24).** `primitives/profile.py`'s `_LEGACY_BUILD_MODE_ALIASES = {"patched_pkgbuild":
  "source_built"}` is a user-writable `profiles.toml` token still honoured on read via
  `normalize_build_mode`, documented as accepted in `docs/design/10-flag-profile-system.md` and
  `11-makepkg-wrapper.md`. It is exactly what row 24 governs, but carries no `Deprecation` record
  and no `warn_used` call — `2.6.1-STD2` catalogued five surfaces and missed this one. The
  `deprecations` bijection cannot catch it: it walks registry→call-site, never code→registry, so a
  compat surface that was never registered is invisible to the gate. Row 24 currently names it as a
  known exclusion; the fix is to add the record (`function=COMPAT`, so `removed_in` must be a
  major) and route `normalize_build_mode`'s alias branch through `warn_used`. Filed separately from
  `2.6.1-F5` because adopting it schedules a removal in the next major, which is a product decision
  rather than bookkeeping. Consider also whether a code→registry direction is addable to
  `check_standards` so the next unregistered alias is found by the tool and not by review.
  *Priority: med · Effort: small · Bump: patch* — the record and warn are trivial; the value is
  closing the blind spot that let this one sit uncatalogued through a major.

- **`2.6.1-B3` — `resolve_repo_mode`'s removed-token `ValueError` reaches the user as a traceback.**
  `2.6.1-F5` made `resolve_repo_mode` raise on `repo_mode = "profiled"` so a stale value fails
  loudly on all six call sites instead of silently resolving to `pacman`. But `verbs/runner.py`
  converts only `RuntimeError` into a clean `_log.error`; everything else propagates verbatim, so
  `sysforge build`, `update` and `reconfigure` print a stack trace. The crafted message ("was
  removed in 3.0.0. Change it to…") is inside that trace, so the user is not left guessing, and it
  matches the pre-existing `_load_packages` precedent — but a config-validation failure should not
  look like a crash. Either map it in the runner or raise a `RuntimeError` subclass the runner
  already understands. *Priority: low · Effort: small · Bump: patch* — cosmetic, but it is the
  first thing a user upgrading with a stale `packages.toml` will see.
