...
|----|------|----------|--------|
| `2.5.1-B12` | A mocked state_dir escapes tmp_path and writes into the repo root | med | small |
...
- **`2.5.1-B12` — A mocked `state_dir` escapes `tmp_path` and writes into the repo root.**
  Running the suite creates `MagicMock/mock.state_dir/<object-id>/` directories in the *working
  tree* — 651 directories and 133 `sentinel_writes.log` files (3.7 MB) had accumulated, and 10
  fresh directories appear on every full run. The path components map exactly to a mock's type
  name, mock name, and `id()`, so a `MagicMock` is reaching code that resolves it as a real
  filesystem path instead of the `state_dir(tmp_path)` fixture (`tests/conftest.py:221`). Two
  consequences: (1) the affected test's sentinel writes never land where it asserts they do, so
  like `2.5.1-B11` it is verifying less than it appears to — the log writes stopped after
  2026-05-31 while the `mkdir` continues, suggesting the assertion path already silently
  changed; (2) the suite mutates the working tree, which is invisible only because
  `.gitignore`'s `*.log` incidentally covers the files (the *directories* match no ignore rule
  at all, so any non-`.log` artifact would surface as untracked and could be committed by a
  broad `git add`). Fix: find the test that passes a mock where a path is expected, route it
  through the `state_dir` fixture, and add a session-scoped guard asserting no `MagicMock/`
  path is created. Do **not** paper over it with a `.gitignore` entry — the directory should
  never exist. *Priority: med · Effort: small* — actively occurring on every run and it
  undermines a test's assertions; the fix is narrow once the writer is identified.
