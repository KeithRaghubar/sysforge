...
- **`2.4.0-F3` — Doctor probe: report which Rust toolchain a build will actually use.** Rust
  ...
  Read-only, so no sentinel; subprocess calls go through the existing seam. *Priority: med · Effort:
  medium* — diagnostic clarity, not a correctness gap, but the nightly-default case is live on this
