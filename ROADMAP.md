...
|----|------|----------|--------|
| `2.6.1-STD1` | Arch-derivative portability as an enforced standard (extend row 23) | med | medium |
...
- **`2.6.1-STD1` — Arch-derivative portability as an enforced standard (extend row 23).** SysForge
  runs on CachyOS today by assertion only at the identity seam: row 23 and its `distro_identity`
  group landed with the container tier (`2.6.1-F2`) — the standards rule forbids a row lagging the
  behaviour, and that change adopted `os-release(5)` — but the row currently asserts **only**
  sub-invariant (c). The other two risk classes have no code-level invariant, and no release gate
  would notice a regression in any of them. Extend row 23, scoped to repo, toolchain-default, and
  version-comparison assumptions on Arch-derived hosts, with the two remaining sub-invariants:
  (a) **no hardcoded sync-repo names**, the `["core", "extra"]` literal at
  ...
  profile-key override intact. Sub-invariant (c) — **distro identity read from `os-release(5)`
  through one primitive**, never `pacman.conf` sniffing, `/etc/arch-release`, or hostname heuristics
  — **already shipped** with F2 (`primitives/os_release.py`, `check_standards` group
  `distro_identity`), so only (a) and (b) remain: extend that group (or add its stated
  `distro_portability` sibling) with static scans in the style of `run_seam` / `privilege_seam`, plus
  cases in `tests/test_standards_compliance.py`; row extension and enforcement land in the same
  commit. Pair it with a per-minor gate: `preflight.sh` section 9 runs the CachyOS container
  ...
  **Unblocked:** `2.6.1-F2` shipped the container harness (`make container-smoke-cachyos`, exit 3 =
  harness unavailable, which is exactly the WARN-on-absence signal section 9 needs) and the
  `os-release` probe (`doctor --distro`). Deliberately *not* in scope: non-Arch-derived distros, any distro-conditional *behaviour* (the row
  forbids assumptions, it does not add per-distro code paths), and the VM-tier concerns the container tier
