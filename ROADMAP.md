...
|----|------|----------|--------|------|
| `3.2.0-B15` | the external-install reconcile matches by exact pkgname, so a manual pacman -S revert of a renamed or split package is silently undone on the next update | high | small | patch |
...
- **`3.2.0-B15` — the external-install reconcile matches by exact pkgname, so a manual `pacman -S`
  revert of a renamed or split package is silently undone on the next `update`.**
  `_reconcile_external_demotions` (`update.py:410`) is the mechanism that makes "user reverted to
  stock" stick: the shipped libalpm hook records every transaction's targets in
  `sentinels/buildstate`, `pacman.batch_install_pkgs` records sysforge's own `pacman -U` targets in
  `sentinels/self-install`, and the difference is the set installed by something other than
  sysforge. `BuildState.reconcile_external_installs` then demotes those entries to inert
  `build_mode = "pacman"` markers. The lookup is a bare `self._data.get(name)`
  (`primitives/build_state.py`), which is the whole defect — it matches neither of the two ways a
  tracked entry's key can differ from the name pacman saw.
  **(a) The `-sysforge` rename.** An optimized build is installed under a renamed pkgname with the
  stock base recorded in `origin_pkgbase`. `pacman -S mesa` puts `mesa` in the external set; the
  tracked key is `mesa-sysforge`. No match, no demotion. The reverse lookup this needs already
  exists as `install_reconcile.resolve_installed_name`, whose docstring names this exact case and
  calls itself the "single home for this reverse lookup" — but it is wired only into `revert_cmd.py:67`
  and `uninstall_cmd.py:45`, never into the reconcile path.
  **(b) Split-package siblings, which is the leg that actually bites.** Conflict-mode renaming
  injects `provides`/`conflicts`/`replaces` for each member's *own* stock name
  (`pkgbuild_patcher.py:1338`), so `pacman -S mesa` displaces `mesa-sysforge` and nothing else —
  `mesa-docs-sysforge` conflicts with `mesa-docs`, which was not a target, so it survives. Its
  build_state entry carries `build_mode = "pgo_mesa"` and `pkgbase = "mesa-sysforge"`, so it is
  still installed, still `source_built`, and `sync_with_installed` has no reason to prune it. The
  next `update` walks it via the `source_built` arm of `update_assemble.py:100`, rebuilds pkgbase
  `mesa-sysforge`, and reinstalls the whole split set — putting the optimized `mesa` back.
  **Reproduced from this workstation's `pacman.log`, not hypothesised:** `2026-07-05T18:55:36`
  removed `mesa-sysforge` and installed stock `mesa`; `2026-07-18` upgraded stock `mesa` normally;
  `2026-07-30T11:11:36` removed `mesa` and installed `mesa-sysforge` again. `mesa-docs-sysforge` is
  present across the entire span and is installed today. Twenty-five days of stock mesa, silently
  reverted by an unattended run.
  Fix is to give the reconcile the same two resolutions the rest of the codebase already has:
  resolve each external name through `resolve_installed_name` before the lookup, then sweep the
  resolved entry's `pkgbase` siblings, which is precisely what `cmd_state_forget`
  (`state_cmd.py`) already does with its `{pn for pn, e in all_pkgs.items() if e.get("pkgbase") == name}`
  set. Both are existing single-home helpers; neither is new logic. Keep the `owner_stage` and
  repo-existence (`3.2.0-B1`) guards as they are. Needs a regression test per leg — a renamed entry
  demoted from its stock name, and a surviving split sibling demoted with its pkgbase — since the
  existing coverage in `tests/test_install_reconcile_resolve.py` exercises the resolver in isolation
  and never through `reconcile_external_installs`.
  *Priority: high · Effort: small · Bump: patch* — high because it is active on a real system and
  silently reverses an explicit user decision with no diagnostic; the change is two existing helpers
  wired into one call site plus its tests.
  **Standards home on adoption:** none new — `install_reconcile`'s module docstring already declares
  itself the single home for the sentinel format and the rename lookup; this makes the reconcile path
  obey the home it already documents.
...
- **`3.2.0-Q2` — what is the unit of "stop maintaining this": a pkgname, a pkgbase, or a policy
  ...
  `3.2.0-B15` fixes the mechanical miss — the reconcile not matching a renamed or split-sibling key.
  It does not settle the model question underneath it, which is what "sysforge no longer owns this
  ...
  **Scope note, checked rather than assumed:** the policy leg is not currently live on this
  ...
  so mesa is tracked purely through build_state and `3.2.0-B15` alone restores correct behaviour
  here. The leg is reachable for anyone who has used `packages add` on a package they later revert,
  ...
  straight off this entry. Related: `3.2.0-B15` is the mechanical prerequisite and should land first
