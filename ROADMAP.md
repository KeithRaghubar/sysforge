...
|----|------|----------|--------|------|
| `3.0.0-B9` | drift-promoted rebuilds never pass -f, so makepkg skips every one of them | high | small | patch |
...
- **`3.0.0-B9` — drift-promoted rebuilds never pass `-f`, so makepkg skips every one of them.**
  `update.py:1021`/`:1044` promote drifted packages from `UP_TO_DATE` to `NEEDS_REBUILD` and stop
  there — no force signal rides along with the promoted `_UpdateResult`. `build_core.py:772`
  (`resolve_cleanbuild_flags`) then computes `batch_flags` **once for the whole batch** as `["-C"]`,
  so the per-target `build_run` at `:840` invokes makepkg without `-f`. Toolchain and flag drift are
  the one rebuild class that happens at an *unchanged* `pkgver`, so the matching artifact is still
  in `PKGDEST` and makepkg exits 13 (`E_ALREADY_BUILT`) → `makepkg_invoke.py:218` raises
  `AlreadyBuilt` → `build_core.py:876` routes it through the hardcoded `"reuse"` posture and
  reinstalls the *stale* artifact — the one carrying the very flags the drift detector objected to.
  The run reports success. Observed 2026-08-16: 62 packages promoted, 52 reused, zero fresh builds.
  Secondary damage — exit 13 also trips `makepkg_wrapper.py:941`, leaving a `PKGBUILD.sysforge`
  sidecar in every affected `~/src` tree.
  **`build` shares the defect**, so the obvious manual fallback fails the same way: `build_cmd.py`
  reads `--force` at `:278` and spends it solely on the repo-package opt-in gate at `:317` — it
  never reaches makepkg — and `:372` hands the same `--makepkg`-only `extra_flags` to the same
  `build_and_install`. `sysforge build --force <pkg>` therefore reads like force-rebuild and is not
  one. (The working incantation today is `--makepkg=-f`, which survives because `-f` is absent from
  `BATCH_STRIP_FLAGS` — `makepkg_flags.py:29,35`.) Land the fix in `build_core` rather than per
  caller: both verbs bottom out in the one loop, so a `force_rebuild` field on the `BuildTarget`
  duck-type serves `update`'s drift promotion and a new `build --rebuild` alike, and fixing only
  `update.py` would leave `build` broken in exactly the case users reach for when `update`
  disappoints them.
  The fix must be **per-target**, not a blanket `-f`:
  `AlreadyBuilt → REUSE` is load-bearing for the resume case (a run interrupted between build and
  install must not rebuild what it already has), so carry a `force_rebuild` flag on the promoted
  result and append `-f` to that target's `extra_flags` alone. With `-f` passed, `AlreadyBuilt` on a
  forced target is unreachable, so make it a hard failure there rather than a silent reuse —
  that assertion is what stops this regressing quietly a second time.
  *Priority: high · Effort: small · Bump: patch* — the headline drift feature is a complete no-op
  and fails silently; the fix is one flag threaded through one existing loop.
  **Standards home on adoption:** none new — `primitives/already_built.py` is already the one home
  for the posture decision; this adds a caller signal, not a second policy site.
