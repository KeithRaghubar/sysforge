...
|----|------|----------|--------|------|
| `3.1.0-B8` | profile CFLAGS overwrite the system conf wholesale, silently dropping Arch's compiler hardening baseline | high | medium | minor |
...
---
- **`3.1.0-B8` — profile `CFLAGS` overwrite the system conf wholesale, silently dropping Arch's compiler hardening baseline.**
  `emit_makepkg_conf` merges `/etc/makepkg.conf` with the resolved profile *per key*, not per token —
  "Profile keys overwrite their system counterparts" (`primitives/makepkg_conf.py:72-74`), implemented
  at `:513-524` where any key present in `profile_overrides` is substituted inline for the system
  value. The shipped profiles set `CFLAGS` in full: `"-march=native -O2 -pipe"`
  (`profiles.toml:228`) and `"-march=native -O3 -pipe -fno-plt"` (`:236`). Arch's stock conf carries
  `-fexceptions -Wp,-D_FORTIFY_SOURCE=3 -Wformat -Werror=format-security -fstack-protector-strong`
  alongside the optimisation flags, so every package built through `standard` or `optimized` compiles
  without run-time buffer-overflow detection, the stack-smashing protector, or the format-string
  rejection the distro applies by default. Nothing re-adds them: `FORTIFY`, `stack-protector`, and
  `format-security` appear nowhere in `sysforge/` outside a `pkgbuild_patcher.py` docstring and test
  fixtures. **This is an oversight, not a policy choice, and the `LDFLAGS` lines prove it** — both
  profiles carefully preserve `-z,relro,-z,now`, the *linker* half of the same hardening set, while
  the *compiler* half was lost rebuilding `CFLAGS` around `-march`/`-O`. The Arch wiki's
  [Security](https://wiki.archlinux.org/title/Security) *Rebuilding packages* section is the
  reference, though the point does not depend on adopting its (disputed, GCC-specific) flag table:
  the defect is that sysforge discards what the system conf already set. Fix in the merge rather than
  by copying tokens into each profile — a profile that omits `CFLAGS` entirely already inherits the
  system value correctly, so the bug is only in the override path. The token-group machinery
  documented at `profiles.toml:294-307` is the natural home: declare the hardening set as a group
  that survives a `CFLAGS` override unless a profile names a conflicting token (`-fno-stack-protector`
  is already in the `stack` group), so opting out stays possible and explicit. Note `is_lib32`
  already special-cases `CFLAGS`/`CXXFLAGS` in this same loop (`makepkg_conf.py:525`), so a
  per-key transform at the override seam is an established shape here.
  *Priority: high · Effort: medium · Bump: minor* — security-bearing and live on every build the tool
  performs, which is as active as a backlog item gets; the merge change is contained to one loop but
  the opt-out key, the group declaration, and the conf-emission tests move with it.
  **Standards home on adoption:** none new — `Arch package guidelines` is the existing standards row
  that governs shipped build flags, and this restores conformance to it rather than adding surface.
