...
def _write_hardware_profile(
    ...
    ``devices`` (a list of ``device_probe.Device``) is emitted as a
    ``[[devices]]`` array-of-tables **after** the scalar ``[hardware]`` and
    ``[kconfig]`` tables — the full PCI/USB inventory with bound-driver state.
    """
    lines = [
...
class HardwareStage(Stage):
    ...
    def run(self, config, state, options):  # noqa: ARG002
        ...
        # --- Write output ---
        _write_hardware_profile(output_path, hw, kconfig, options.dry_run, devices)
