...
"""
pipeline_state.py — pipeline checkpoint state
...
Lives in the leaf layer, not under ``pipeline/`` (3.2.0-F12). The class is the
single home for ``pipeline_state.toml``'s format, and leaf-layer code needs to
read it — ``primitives/init_notice.py`` asks whether the bootstrap stages are
done — so filing it above the leaf layer forced a reach-up that no amount of
duplication could fix without splitting that one home. ``pipeline.state``
re-exports every public name for one cycle so existing imports keep working;
new callers should import from here directly.
...
# Path resolution
# ---------------------------------------------------------------------------
# ``resolve_state_dir`` moved down to ``primitives/paths.py`` in 3.2.0-F1a. It
# is re-exported here (and, in turn, from ``pipeline.state``) so the long tail
# of ``from sysforge.pipeline.state import resolve_state_dir`` imports — and the
# tests that patch that name — keep working for one cycle.
