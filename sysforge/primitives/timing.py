...
def render_report(timer: PhaseTimer, *, title: str = "Phase timings") -> list[str]:
    ...
    if not timer.records:
        return []
    width = max(len(r.name) for r in timer.records)
    lines = [f"{title}:"]
    lines.extend(
        f"  {r.name.ljust(width)}  {_fmt_ms(r.duration_ms)}" for r in timer.records
    )
    lines.append(f"  {'total'.ljust(width)}  {_fmt_ms(timer.total_ms())}")
    return lines
