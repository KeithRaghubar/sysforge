...
def cmd_state_forget(args):
    """``sysforge state forget`` — demote the named package(s) and report.

    Thin adapter over :func:`sysforge.verbs.shared.forget_packages`, which holds
    the operation itself because ``revert`` and ``uninstall`` demote too
    (3.2.0-F8). This function owns only the argument unpacking and the
    verb's own output; the other two callers fold demotion into their summaries.
    """
    from sysforge.primitives.paths import resolve_state_dir
    from sysforge.verbs.shared import forget_packages

    state_dir, _ = resolve_state_dir(getattr(args, "state_dir", None))
    forgotten, missing = forget_packages(state_dir, getattr(args, "pkgnames", None) or [])
